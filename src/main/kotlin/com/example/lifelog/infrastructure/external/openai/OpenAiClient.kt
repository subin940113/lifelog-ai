package com.example.lifelog.infrastructure.external.openai

import com.example.lifelog.infrastructure.config.OpenAiProperties
...
class OpenAiClient(
    ...
    private val timeout = Duration.ofSeconds(properties.timeoutSeconds)
    ...
    fun structure(
        ...
        val cacheKey = cacheKey(kind = "plain", system = system, user = user, schemaJson = null)
        return callChatCompletions(request, cacheKey)
    }
    ...
    fun structureWithSchema(
        ...
        val cacheKey = cacheKey(kind = "schema", system = system, user = user, schemaJson = schemaJson)
        return callChatCompletions(request, cacheKey)
    }
    ...
    private fun callChatCompletions(
        ...
        try {
            val content = withShortRetry { executeOnce(request) }
    ...
    private fun executeOnce(request: ChatCompletionsRequest): String =
        try {
            client
                .post()
                ...
                .exchangeToMono { response ->
                    ...
                    if (statusCode == 429) {
                    ...
                    } else if (statusCode in 500..599 || statusCode == 408 || statusCode == 409) {
                        response
                            .bodyToMono(String::class.java)
                            .defaultIfEmpty("")
                            .flatMap { body ->
                                Mono.error(OpenAiTransientException("OpenAI transient http=$statusCode bodySnippet=${body.take(300)}"))
                            }
                        ...
                        response
                            ...
                            .defaultIfEmpty("")
                            .flatMap { body ->
                                Mono.error(OpenAiPermanentException("OpenAI permanent http=$statusCode bodySnippet=${body.take(500)}"))
                            }
                    } else {
                        response.bodyToMono(ChatCompletionsResponse::class.java)
                    }
                }.timeout(timeout)
                .map { parsedResponse ->
                    val choice =
                        parsedResponse.choices.firstOrNull()
    ...
    private fun <T> withShortRetry(block: () -> T): T {
        ...
        while (attempt < maxAttempts) {
            try {
                ...
                if (exception is OpenAiRateLimitedException) throw exception
                if (exception is OpenAiBusyException) throw exception
                if (exception is OpenAiPermanentException) throw exception
                if (exception !is OpenAiTransientException) throw exception
        ...
        throw OpenAiTransientException(
            message = "OpenAI 호출 재시도 초과 (attempts=$maxAttempts). last=${lastException?.message}",
            cause = lastException,
        )
    }
    ...
    private fun computeCooldownUntil(retryAfterSeconds: Long?): Instant {
        ...
        val consecutive429Count = consecutive429.incrementAndGet()
        val baseSeconds =
            when {
                consecutive429Count <= 1 -> 120L
                consecutive429Count == 2 -> 300L
                else -> 900L
            }
